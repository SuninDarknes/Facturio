...
export default function DialogDodajPrimku({
    isOpen,
    onClose,
    onSuccess,
}: DialogDodajPrimkuProps) {
    ...
    const { data, setData, post, processing, errors, reset } = useForm<PrimkaData>({
        naziv: '',
        dobavljac_id: '',
        datum: '',
        pdv: '',
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        post(route('primke.store'), {
            onSuccess: () => {
                reset();
                onClose();
                onSuccess('Primka je uspješno dodana.');
            },
            onError: () => {
                console.log(errors);
            }
        ...
        <>
            <Dialog open={isOpen} onOpenChange={onClose}>
                <DialogContent className="max-w-300"> {/* Dodajte klasu za širinu */}
                    <DialogHeader>
                        <DialogTitle>Dodaj novu primku</DialogTitle>
                    </DialogHeader>
