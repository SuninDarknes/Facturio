...
import { Osoba, Artikl, PageProps } from '@/types';
import DialogDodajOsobu from './DialogDodajOsobu';
import DialogDodajArtikl from './DialogDodajArtikl'; // Import dijaloga za dodavanje artikla
import { useSpring, animated } from '@react-spring/web';
import { SpaceIcon } from 'lucide-react';

interface DialogDodajRacunProps {
    isOpen: boolean;
    onClose: () => void;
...
export default function DialogDodajRacun({
    isOpen,
    onClose,
    onSuccess,
}: DialogDodajRacunProps) {
    const [isOsobaDialogOpen, setIsOsobaDialogOpen] = useState(false);
    const [isArtiklDialogOpen, setIsArtiklDialogOpen] = useState(false);
    const [ukupnaCijena, setUkupnaCijena] = useState<number>(0);
    const { osobe, artikli } = usePage<PageProps>().props; // Osobe i artikli se automatski osvježavaju iz backend-a
    const { data, setData, post, processing, errors, reset } = useForm<RacunData>({
        naziv: '',
        osoba_id: '',
        datum: '',
        pdv: '',
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        post(route('racuni.store'), {
            onSuccess: () => {
                reset();
                onClose();
                onSuccess('Račun je uspješno dodan.');
            },
            onError: () => {
                console.log(errors);
            }
    ...
    const handleOsobaSuccess = () => {
        setIsOsobaDialogOpen(false); // Zatvori dijalog za dodavanje osobe
    };
    ...
    return (
        <>
                            ...
                            <div className='grid grid-cols-3 gap-4'>
                                ...
                                <div>
                                    ...
                                    <Select
                                        ...
                                        onValueChange={(value) => {
                                            if (value === 'novi') {
                                                setIsOsobaDialogOpen(true); // Otvori dijalog za dodavanje osobe
                                            } else {
                                                setData('osoba_id', value);
                                            }
                                        }}
                                    >
                                        <SelectTrigger>
                                        ...
                                        <SelectContent>
                                            {osobe.map((osoba) => (
                                                <SelectItem key={osoba.id} value={osoba.id.toString()}>
                                                    {osoba.ime + " " + osoba.prezime}
                                                </SelectItem>
                                            ))}
                                            <SelectItem value="novi">+ Dodaj novu osobu</SelectItem>
                                        </SelectContent>
                                    </Select>
                                    {errors.osoba_id && (
                                        <span className="text-red-500 text-sm">{errors.osoba_id}</span>
                                    )}
                                </div>
                                    ...
                                    <div>
                                        ...
                                        <div className='grid grid-cols-2 gap-4'>
                                            ...
                                            <div>
                                                ...
                                                <Input type='text' value={data.nacin_placanja} onChange={(e) => setData('nacin_placanja', e.target.value)}
                                                />
                                                {errors.nacin_placanja && (
                                                    <span className="text-red-500 text-sm">{errors.nacin_placanja}</span>
                                                )}
                                            </div>
                                        ...
                                        <div>
                                                ...
                                                <Input type='date' value={data.rok_placanja} onChange={(e) => setData('rok_placanja', e.target.value)}
                                                />
                                                {errors.rok_placanja && (
                                                    <span className="text-red-500 text-sm">{errors.rok_placanja}</span>
                                                )}
                                            </div>
            ...
            <DialogDodajOsobu
                isOpen={isOsobaDialogOpen}
                onClose={() => setIsOsobaDialogOpen(false)}
